...
namespace TinyMoneyManager.ViewModels
{
    ...
    public class TransferingHistoryViewModel : NkjSoftViewModelBase
    {
        ...
        private ObservableCollection<TransferingItem> transferingItemList;

        public ObservableCollection<TransferingItem> TransferingItemList
        {
            get { return transferingItemList; }
            set
            {
                if (transferingItemList != value)
                {
                    OnNotifyPropertyChanging("TransferingItemList");
                    transferingItemList = value;
                    OnNotifyPropertyChanged("TransferingItemList");
                }
            }
        }


        private TinyMoneyDataContext TransferingHistoryContext;
        ...
        public void LoadData(Func<TransferingItem, bool> filterSelector)
        {
            ...
            Thread th = new Thread((o) =>
            {
                ...
                {
                    query = TransferingHistoryContext.TransferingItems
                ...
                query.OrderByDescending(p => p.TransferingDate).ForEach(p =>
                        Deployment.Current.Dispatcher.BeginInvoke(() => TransferingItemList.Add(p)));
        ...
        {
            var query = TransferingHistoryContext.TransferingItems
                     .Where(filterSelector);
            return query;
        }
        ...
        {
            var temp = TransferingItemList.FirstOrDefault(p => p.Id == id);
            TransferingItemList.Remove(temp);
            TransferingHistoryContext.TransferingItems.DeleteOnSubmit(temp);
