    ...
    public class DataContextDataHandler
    {
        ...
        public void backupData(out string contentOfXml)
        {
            ...
            try
            {
                ...
                {
                    foreach (System.Collections.Generic.KeyValuePair<String, Func<TinyMoneyDataContext, DataSynchronizationInfo, String>> pair in this.TableBackupImage)
                    {
        ...
        public DataSynchronizationInfo BackupDataToLocal()
        {
            ...
            userStoreForApplication.CreateDirectorySafe(LocalDataFolderName);
            System.IO.IsolatedStorage.IsolatedStorageFileStream stream = userStoreForApplication.OpenFile(LocalFileName, System.IO.FileMode.OpenOrCreate, System.IO.FileAccess.Write);
        ...
        public void RestoreData(string dataForContext)
        {
            ...
            {
                MessageBox.Show(LocalizedStrings.GetLanguageInfoByKey("ErrorMessageWhenRestoreEmptyDataFailed").FormatWith(new object[] { "http://yurukai.wordpress.com", exception.Message }), App.AlertBoxTitle, MessageBoxButton.OK);
        ...
        public DataSynchronizationInfo RestoreDataFromLocal()
        {
            System.IO.IsolatedStorage.IsolatedStorageFile userStoreForApplication = System.IO.IsolatedStorage.IsolatedStorageFile.GetUserStoreForApplication();
            string path = this.EnsureFileToUse();
            if (!userStoreForApplication.FileExists(path))
            {
                throw new System.IO.FileNotFoundException(path);
            }
            System.IO.IsolatedStorage.IsolatedStorageFileStream stream = userStoreForApplication.OpenFile(path, System.IO.FileMode.Open);
            ...
            if (this.DataSynchronizationDataArg.Result == OperationResult.Successfully)
            {
                try
                {
                    userStoreForApplication.DeleteFile(LocalFileName);
                }
                catch (System.Exception exception)
                {
