namespace TinyMoneyManager.ViewModels.DataSyncing
{
    ...
    public class SkyDriveDataSyncingViewModel : NkjSoftViewModelBase
    {
        ...
        public async void CheckFolderStructureOnServer()
        {
            this.IsBusy = true;
            GlobalIndicator.Instance.BusyForWork(AppResources.DectingDataSyncingRootFolder, new object[0]);
            try
            {
                var result = await this.LiveConnector.GetAsync("/me/skydrive/files");

                this.LiveConnector_GetRootFolderCompleted(result);
            }
        ...
        private void LiveConnector_GetRootFolderCompleted(ConfiguredTaskAwaitable<LiveOperationResult> result)
        {
        ...
        public System.Collections.Generic.List<ObjectFromSkyDrive> DectingDataSyncingRootFolder(LiveOperationResult e)
        {
        ...
        public async Task<LiveDownloadOperationResult> DownloadFile(string folderFrom, string fileId)
        {
            string workText = AppResources.StepInfo_TransferingData;
            GlobalIndicator.Instance.BusyForWork(workText, new object[0]);
            try
            {
                //this.LiveConnector.GetCompleted += LiveConnector_GetCompleted;

                //this.LiveConnector.GetAsync(fileId + "/content?return_ssl_resources=true", callBack);

                var result = await this.LiveConnector.DownloadAsync(fileId + "/content?return_ssl_resources=true");
        ...
        private void LiveConnector_GetRootFolderCompleted(LiveOperationResult e)
        {
                ...
                if (!this.HasRootDirFolder)
                {
                    if (isOkToDo == null)
                    {
                        isOkToDo = async delegate
                        {
                            ...
                            var postResult = await this.LiveConnector.PostAsync("me/skydrive", body);

                            LiveConnector_PostCreateRootFolderCompleted(postResult);
                        };
                    }
        ...
        private void LiveConnector_PostCreateRootFolderCompleted(LiveOperationResult e)
        {
            if (!this.HandleError(null, AppResources.CreatingFolder))
            {
        ...
        private void LiveConnector_UploadFileCompleted(LiveOperationResult e, System.Action<IDictionary<String, Object>> callBack = null)
        {
            if (!this.HandleError(null, AppResources.StepInfo_TransferingData))
            {
                if (callBack != null)
                {
                    callBack(e.Result);
        ...
        public async Task<bool> UploadFile(string folderTo, string fileName, System.IO.Stream fileContent, System.Action<IDictionary<String, Object>> callBack = null)
        {
            string workText = AppResources.StepInfo_TransferingData;
            GlobalIndicator.Instance.BusyForWork(workText, new object[0]);
            try
            {
                var result = await this.LiveConnector.UploadAsync(folderTo, fileName, fileContent, OverwriteOption.Overwrite);


                LiveConnector_UploadFileCompleted(result, callBack);
