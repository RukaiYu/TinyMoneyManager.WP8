...
namespace TinyMoneyManager.Data
{
    ...
    public class DataSynchronizationHandler : NkjSoftViewModelBase
    {
        ...
        public static string DatabaseFileToBackup { get; set; }
        ...
        public DataBackupedFromPhone BackupData(BackupDataOption option, Action<DataSynchronizationInfo> callBack)
        {
            ...
            if (ToBackupDataBase)
            {
                result.SdfFileContent = ExtractDataFromSdb(option.DbFileNameTobackup);

                if (option.DbFileNameTobackup != TinyMoneyDataContext.DbFileName)
                {
                    result.CategoryListXmlSource = System.IO.Path.GetFileName(option.DbFileNameTobackup);
                }
                else
                {
                    result.CategoryListXmlSource = "accountBook_database_v{0}.0.sdf".FormatWith(DatabaseVersion.CurrentVersion);
                }

                result.AppSettingXmlSource = "This is sdf file content. only for database version : " + DatabaseVersion.CurrentVersion;
            }
            else
            {
